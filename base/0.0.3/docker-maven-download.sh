...
maven_apicurio_converter() {
    if [[ -z "$KAFKA_CONNECT_PLUGINS_DIR" ]] ; then
        echo "WARNING: KAFKA_CONNECT_PLUGINS_DIR is not set. Skipping Apicurio converter loading..."
        return
    fi
    if [[ ! -d "$KAFKA_CONNECT_PLUGINS_DIR" ]] ; then
        echo "WARNING: KAFKA_CONNECT_PLUGINS_DIR is not a directory. Skipping Apicurio converter loading..."
        return
    fi
    ...
    mkdir "$KAFKA_CONNECT_PLUGINS_DIR/apicurio"
